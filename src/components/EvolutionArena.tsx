...
import {derivePhaseState} from "../turn-state";
...
function initialGuidedMode(): boolean {
  const requestedMode = new URLSearchParams(globalThis.location.search).get("mode");
  if (requestedMode === "advanced") return false;
  if (requestedMode === "guided") return true;
  try {
    return globalThis.localStorage.getItem("ets2:interface-mode") !== "advanced";
  } catch {
    return true;
  }
}
...
export default function EvolutionArena({
  ...
  const [guided, setGuided] = useState(initialGuidedMode);
  ...
  const pendingPortraits = planet.yourSpecies.filter((species) => {
    ...
    return node && ["pending", "rejected"].includes(node.portrait.status);
  });
  ...
  function openActionLab(speciesId: string) {
    ...
    globalThis.requestAnimationFrame(() => {
      globalThis.document.getElementById("mutation-lab")?.scrollIntoView({behavior: "smooth", block: "nearest"});
    });
  }
  ...
  function selectTreeNode(nextSelection: AncestrySelection) {
    ...
    setShowDossier(nextSelection.nodeId !== "next");
  }
  ...
  function toggleInterfaceMode() {
    setGuided((current) => {
      const next = !current;
      try {
        globalThis.localStorage.setItem("ets2:interface-mode", next ? "guided" : "advanced");
      } catch {
  ...
  const lockedMessage = planet.phase === "reveal"
    ...
    : phaseState.locked
      ...
      : phaseState.expired
        ? "This phase expired. Use the phase console to advance."
  ...
  return (
    <main className={`game-cockpit biome-${planet.biome} ${guided ? "guided-mode" : "advanced-mode"} ${labOpen ? "lab-open" : "lab-closed"}`} id="top">
      ...
      <header className="hud-topbar">
        ...
        <div className="hud-player-controls">
          ...
          <button className="interface-toggle" type="button" aria-pressed={guided} onClick={toggleInterfaceMode}>{guided ? "FULL UI" : "GUIDED"}</button>
          <div className="commander-chip"><span><i />STUDIONET</span><b>{shortAddress(address)}</b></div>
        </div>
      </header>

      <aside className="ecosystem-rail hud-panel">
        <header><div><span className="hud-kicker">Your ecosystem</span><h2>Species roster</h2></div><strong>{livingOwned.length}<small> / {planet.maxSpeciesPerWallet}</small></strong></header>
        <div className="species-slots">
          {planet.yourSpecies.map((species, index) => {
            ...
            return (
              <article key={species.speciesId} className={`roster-species ${selected ? "selected" : ""} ${species.alive ? "living" : "retired"}`}>
                ...
                {species.alive && portraitNeedsWork ? <button className="roster-portrait-command" type="button" disabled={Boolean(busy)} onClick={() => void onGeneratePortrait(species.speciesId)}>{busy === `portrait-${species.speciesId}` ? "VERIFYING…" : node.portrait.status === "rejected" ? "RETRY PORTRAIT" : "CREATE PORTRAIT"}</button> : null}
              </article>
            );
          })}
      ...
      <section className="ancestry-theater">
        {planet.hazard && forecast ? (
          <div className={`hazard-alert ${forecast.status}`}>
            <span className="hazard-glyph">!</span>
            <div><small>ERA {planet.era} EXTINCTION EVENT</small><strong>{planet.hazard.name}</strong><p>{planet.hazard.description}</p></div>
            <div className="hazard-trait"><small>Required</small><b>{label(planet.hazard.adaptationClass)} {planet.hazard.threshold}</b></div>
            <div className="hazard-subject"><small>{selectedSpecies?.name}</small><b>{forecast.value} / {planet.hazard.threshold}</b><span>{forecast.populationLoss === 0 ? "SURVIVAL LIKELY" : `PROJECTED −${forecast.populationLoss}m`}</span></div>
            <div className="hazard-meter"><i style={{width: `${Math.min(100, forecast.value / planet.hazard.threshold * 100)}%`}} /></div>
          </div>
        ) : null}
      ...
      <aside className="intel-rail">
        <TurnDirector planet={planet} address={address} state={phaseState} secrets={secrets} busy={busy} guided={guided} pendingPortrait={pendingPortraits[0] ?? null} nextEvolvableSpecies={nextEvolvableSpecies} lastSyncedAt={lastSyncedAt} onLock={onLock} onReveal={onReveal} onAdvance={onAdvance} onCopyBackup={onCopyBackup} onRestoreBackup={onRestoreBackup} onGeneratePortrait={onGeneratePortrait} onEvolveSpecies={openActionLab} onRefresh={() => void onRefresh()} />
        {selectedSpecies && (!guided || showDossier) ? <AncestryDossier species={selectedSpecies} selection={selection} isYou={selectedSpecies.owner === viewer} canEvolve={evolvableSpeciesIds.has(selectedSpecies.speciesId)} lockedMessage={lockedMessage} onEvolve={openActionLab} /> : null}
      </aside>
      ...
      <section className={`mutation-deck ${phaseState.canCommit ? "online" : "offline"}`} id="mutation-lab">
        <header className="mutation-deck-header">
          ...
          {guided ? <ol className="guided-lab-roadmap" aria-label="Evolution steps"><li className="done">Action</li><li>Species</li><li>Genes</li><li>Explain</li><li>Seal</li></ol> : null}
          <div className="mutation-deck-tools"><span className="deck-energy">ENERGY {Array.from({length: planet.evolutionEnergyPerEra}, (_, index) => <i key={index} className={index < phaseState.remainingEnergy ? "charged" : "spent"} />)}</span>{guided ? <button className="guided-lab-close" type="button" onClick={() => setLabOpen(false)} aria-label="Close evolution controls">CLOSE ×</button> : null}</div>
        </header>
        ...
        {phaseState.canCommit ? (
          ...
          <div className="deck-lockdown">
            ...
            <span><small>CURRENT OBJECTIVE</small><h3>{lockdownObjective}</h3><p>The phase console on the right shows exactly who is ready and the next command available.</p></span>
